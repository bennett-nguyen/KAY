...
def _compute_final_coordinates(node: Node, mod_sum: float):
    ...
    node.x = node.preliminary_x * const.SCALE
    node.y = (node.depth + const.DEPTH_OFFSET) * const.VERTICAL_SCALE
...
def _get_contour(node: Node, mod_sum: float, values: dict[int, float], side: str):
    side = side.lower()
