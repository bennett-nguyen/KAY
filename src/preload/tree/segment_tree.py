...
class SegmentTree:
    ...
    def array_length(self) -> int:
        ...
        This property returns the number of elements in the array associated with the segment tree. It provides a convenient way to access the size of the array without directly interacting with it.
    ...
    def query(self, q_low: int, q_high: int) -> int:
        ...
        This function queries the segment tree to compute the aggregate value for the elements within the range defined by `q_low` and `q_high`. It delegates the actual querying logic to a private method that handles the specifics of the segment tree traversal.
    ...
    def update(self, pos: int, val: int) -> None:
        ...
        This function modifies the value of an element in the segment tree at the given position. It delegates the actual update logic to a private method that handles the specifics of the segment tree traversal and value adjustment.
    ...
    def rebuild(self):
        ...
        This function initializes a new root node for the segment tree and constructs the tree structure based on the current array. It ensures that the segment tree is updated to reflect any changes in the underlying data.
        """
        self.root = Node()
        self._build(self.root, 0, self.array_length-1)
    ...
    def _build(self, node: Node, low: int, high: int, ID: int = 1) -> None:
        ...
        This function constructs the segment tree by initializing nodes and setting their values based on the specified range. It divides the range into subranges and recursively builds the left and right child nodes, combining their values using the specified function.
    ...
    def _update(self, pos: int, val: int, node: Node, low: int, high: int) -> None:
        ...
        This function modifies the value of a node in the segment tree at the given position and updates the relevant parent nodes accordingly. It traverses the tree to find the correct node to update based on the specified position and ensures that the current node's value is updated after the change.
    ...
    def _query(self, q_low: int, q_high: int, node: Node, low: int, high: int) -> int:
        ...
        This function retrieves the aggregate value for the elements within the range defined by `q_low` and `q_high`. It checks for invalid queries and determines if the current node's range is fully within the query range, returning the node's data if so, or recursively querying the left and right children otherwise.
    ...
    def _is_query_invalid(self, q_low: int, q_high: int, low: int, high: int) -> bool:
        ...
        This function determines whether the specified query range is valid by checking if the lower bound exceeds the upper bound or if the query range does not overlap with the current range. It returns a boolean indicating the validity of the query.
    ...
    def _is_query_within_range(self, q_low: int, q_high: int, low: int, high: int) -> bool:
        ...
        This function determines whether the specified range defined by `low` and `high` is completely contained within the query range defined by `q_low` and `q_high`. It returns a boolean indicating if the current range is within the bounds of the query.
